...
impl DeviceDriver for LogitechG300Driver {
    ...
    async fn load_profiles(&mut self, io: &mut DeviceIo, info: &mut DeviceInfo) -> Result<()> {
        ...
        for profile_id in 0..=LOGITECH_G300_PROFILE_MAX {
            let mut profile = crate::engine::device::ProfileInfo {
                index: profile_id,
            ...
            for res_id in 0..LOGITECH_G300_NUM_DPI {
                profile.resolutions.push(crate::engine::device::ResolutionInfo {
                    index: res_id,
                    is_active: profile_id == active_idx as u32 && res_id == active_res as u32,
                    is_default: false,
                    is_disabled: false,
                    dpi: crate::engine::device::Dpi::Unknown,
                    dpi_list: vec![],
                    capabilities: Vec::new(),
                });
            }
            ...
            for btn_id in 0..=LOGITECH_G300_BUTTON_MAX {
                profile.buttons.push(crate::engine::device::ButtonInfo {
                    index: btn_id,
                    action_type: crate::engine::device::ActionType::Unknown,
            ...
            profile.leds.push(crate::engine::device::LedInfo {
                index: 0,
                mode: crate::engine::device::LedMode::Solid,
                modes: vec![],
                color: crate::engine::device::Color::default(),
                secondary_color: crate::engine::device::Color::default(),
                tertiary_color: crate::engine::device::Color::default(),
                color_depth: 1,
                effect_duration: 0,
                brightness: 255,
            });
        ...
        for profile in &info.profiles {
            ...
            for btn in &profile.buttons {
                ...
                match btn.action_type {
                    crate::engine::device::ActionType::Button => {
                        let val = btn.mapping_value as u8;
                        if (1..=9).contains(&val) {
                            data.code = val;
                        }
                    }
                    crate::engine::device::ActionType::Special => {
                    ...
                    crate::engine::device::ActionType::Key | crate::engine::device::ActionType::Macro => {
                        data.code = 0x00;
